...
sum_fd_stencil=sum(abs(coeff(spatial_order)))
theta=arange(0,2*pi,0.01)
print("You choose a spatial order of ",spatial_order)

## Temporal 2 order (Leapfrog)
f=-1j*(((exp(-1j*theta))**(2.5)-(exp(-1j*theta))**1.5))
f2=(2*sum_fd_stencil)
f_ges=f/f2
print("2. Temporal Order has the stability limit CFL=",max(f_ges.real))

## Temporal 3 order (Adams-Bashforth method)
f=-1j*(((exp(-1j*theta))**2.5-(exp(-1j*theta))**1.5)*1);
f2=(2*sum_fd_stencil*(25.0/24.0*(exp(-1j*theta))**2
    -1./12.*(exp(-1j*theta))**1+1./24.))
f_ges=f/f2
print("3. Temporal Order (ABS-method) has the stability limit CFL=",max(f_ges.real))

## Temporal 4 order (Adams-Bashforth method)
f=-1j*(((exp(-1j*theta))**3.5-(exp(-1j*theta))**2.5))
f2=(2*sum_fd_stencil*(13./12.*(exp(-1j*theta))**3
    -5./24.*(exp(-1j*theta))**2+1./6.*(exp(-1j*theta))**1-1./24.))
f_ges=f/f2
print("4. Temporal Order (ABS-method) has the stability limit CFL=",max(f_ges.real))
